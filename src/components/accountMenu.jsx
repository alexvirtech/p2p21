...
const AccountMenu = ({ close }) => {
    ...
    useEffect(() => {
        console.log(state.account.name)
    }, [])
    ...
    return (
        <LayoutMenu title="123" close={() => close()}>
            <div class="flex flex-col w-64 bg-gray-800 absolute right-0 top-16 z-10">
                    ...
                    <div class="">
                        ...
                        >
                            {state.accounts.map((a, i) => (
                                <option key={i} value={a.name} selected={a.name === state.account.name}>
                                    {a.name}
                                </option>
                            ))}
                        </select>
                ...
                {actions.map((item, index) => (
                    <> {/* this condition is wrong */}
                        {((tempName.current && tempName.current?.value !== defAccount) || state.account.name !== defAccount || item.custom) && (
            ...
            {state.modal === "password" && (
                <Password name={tempName.current?.value} title="Enter Password" close={closePassword} />
            )}
        </LayoutMenu>
    )
}
